...
double* doFit(string condition, string muonId, const char* savePath = NULL)
{
	...
	// CHI2 test
	//RooChi2Var chi2_all("chi2_all", "chi2_all", model_all, dh_ALL, true, RooAbsData::Poisson);
	//RooChi2Var chi2_pass("chi2_pass", "chi2_pass", model_pass, dh_PASS, true, RooAbsData::Poisson);
	...
	//TLatex latexAll;
	//latexAll.DrawLatex(.7, .6, ("#chi^2/ndf = " + to_string(chi2_all.getVal())).c_str()
	...
	model_pass.plotOn(frame_pass, RooFit::Components("GS"), Name("GS"), RooFit::LineStyle(kDashed), RooFit::LineColor(kGreen));
	model_pass.plotOn(frame_pass, RooFit::Components("CB"), Name("CB"), RooFit::LineStyle(kDashed), RooFit::LineColor(kMagenta - 5));
	...
	//TLatex latexPass;
	//latexPass.DrawLatex(.7, .6, ("#chi^2/ndf = " + to_string(chi2_pass.getVal())).c_str());
