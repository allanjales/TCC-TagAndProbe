...
double* doFit(string condition, string MuonId, const char* savePath = NULL)
{
	...
	RooDataSet *Dataset = new RooDataSet("DATA", "DATA", 
	RooArgSet(MuonId_var, InvariantMass, ProbeMuon_Pt, ProbeMuon_Eta, ProbeMuon_Phi, TagMuon_Pt, TagMuon_Eta, TagMuon_Phi),
	Import(*DataTree));

	RooFormulaVar* fv_CUT   = new RooFormulaVar("tag_cut", "TagMuon_Pt >= 7.0 && fabs(TagMuon_Eta) <= 2.4", *Dataset->get());
	RooDataSet*    Data_CUT = new RooDataSet("data_cut", "data_cut", Dataset, *Dataset->get(), *fv_CUT);

	RooFormulaVar* fv_ALL   = new RooFormulaVar("probe_on_bin", condition.c_str(), *Data_CUT->get());
	RooDataSet*    Data_ALL = new RooDataSet("data_all", "data_all", Data_CUT, *Data_CUT->get(), *fv_ALL);

	RooFormulaVar* fv_PASS      = new RooFormulaVar("passing_probe_on_bin", (MuonId_str + "==1").c_str(), *Data_ALL->get());
	RooDataSet*    Data_PASSING = new RooDataSet("data_pass", "data_pass", Data_ALL, *Data_ALL->get(), *fv_PASS);
